...
from app.schemas.requests import (
    ChangePasswordRequest, Enable2FAResponse, LoginRequest, PasswordResetConfirm,
    PasswordResetRequest, RefreshRequest, RegisterRequest, TokenPair, Verify2FARequest,
)
...
async def register(body: RegisterRequest, db: AsyncSession = Depends(get_db)) -> TokenPair:
    ...
    AuditLog.record("user_registered", actor_id=pseudonymize(user.id), tenant_id=org.id,
...
async def login(body: LoginRequest, db: AsyncSession = Depends(get_db)) -> TokenPair:
    ...
    AuditLog.record("login", actor_id=pseudonymize(user.id), tenant_id="",
                    resource_type="user", resource_id=pseudonymize(user.id))
    return TokenPair(access_token=access, refresh_token=refresh, expires_in=900)
...
async def logout(user: CurrentUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> None:
    svc = AuthService(db)
...
async def password_reset_request(body: PasswordResetRequest, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    # In production, send token by email. Never reveal whether email exists.
